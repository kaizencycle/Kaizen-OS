...
**Cycle:** C-284
**Status:** Protocol Spec v2 · Canonical
**Author:** kaizencycle
**Predecessor:** [Vault-to-Fountain Protocol v1](./VAULT_TO_FOUNTAIN_PROTOCOL.md)
**Implementation:** `mobius-civic-ai-terminal/lib/vault-v2/*`
**CC0 Public Domain**
...
- preserve full doctrinal continuity with v1 — *reserve becomes flow only
  when integrity proves it can hold the weight*
...
- **A Seal carries its own integrity record.** The Seal remembers the GI,
  ...
  decisions consult the Seal's own conditions, not just the present's.
...
Beyond v1 §7 rules, Vault v2 adds:
...
Lives in `mobius-civic-ai-terminal`:
...
Canonical protocol doc: **this file**.

---

## 13. Archive path (Substrate mirror)

Attested Seals are mirrored from Terminal KV to this Substrate repo via a
daily archive cron (scoped for v2.1). The target path is:

```
journals/vault/seals/{seal_id}.json
```

The Substrate mirror is the **cold-truth layer**. KV is the **hot-truth
layer**. They must agree; if they disagree, ZEUS is raised.
