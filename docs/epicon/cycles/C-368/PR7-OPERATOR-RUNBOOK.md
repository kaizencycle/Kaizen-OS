...
**Witnessed gap (2026-07-10):** 349 sealed blocks in terminal KV · 0 `.dat` files on Substrate main.
...
## Phase A+B — One-shot prime (349 blocks)
...
**Acceptance:** `./docs/epicon/cycles/C-368/c368-verify.sh pr7` passes (≥1 `.dat`, MANIFEST with `total_blocks` ≥ 349).

## Phase C — Continuous append (after prime)

- **Vercel cron:** `/api/cron/reserve-canon-append` daily at `00:30 UTC`
- **GitHub schedule:** same workflow at `30 0 * * *` (skips when gap = 0)
