...
- **`/api/vault/global` vs Terminal `vault/status`:** both can report `vault_id: "vault-global"` while sealed totals disagree (ledger `0` blocks / `$0` vs Terminal `360` / `18000` MIC-class reserve). That is **`vault_witness_divergence`** — same declared identity, contradictory truths (closer to KV collision class than GI spread).
...
- **#20:** `python3 scripts/c383_reconciliation.py` — [RECONCILIATION_REPORT.md](./RECONCILIATION_REPORT.md). Emit **`gi_source_unwired`** and **`vault_witness_divergence`** as distinct rows (vault: same `vault_id`, contradictory sealed state).
...
| Ledger `vault-global` empty vs Terminal vault activity | TRUE | `curl` `/api/vault/global` vs Terminal `vault/status` — same `vault_id`, contradictory sealed totals; `scripts/c383_reconciliation.py` |
