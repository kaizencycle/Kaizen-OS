...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const [waitingBefore, delayedBefore] = await Promise.all([
      queue.getWaitingCount(),
      queue.getDelayedCount()
    ]);
    
    const totalToDrain = waitingBefore + delayedBefore;
    ...
    return res.status(200).json({
      ok: true,
      message: `Drained ${totalToDrain} jobs from queue`,
      drainedCount: totalToDrain,
      stats: {
        waiting,
        active,
        delayed,
    ...
    return res.status(500).json({ 
      ok: false, 
      error: error?.message || "queue_drain_failed" 
    });
  }
}
